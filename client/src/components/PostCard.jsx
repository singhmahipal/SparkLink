...
const PostCard = ({post}) => {
    ...
    // Add safety check for post.content
    ...
    // Add safety check for post.user
    if (!post.user) {
  ...
  return (
    <div className='bg-white rounded-xl shadow p-4 space-y-4 w-full max-w-2xl'>
        ...
        <div onClick={() => navigate(`/profile/` + post.user._id)} className="inline-flex items-center gap-3 cursor-pointer">
            <img 
                src={post.user.profile_picture || dummyUserData.profile_picture} 
                alt="profile pic" 
            ...
            <div>
                ...
                <div className='text-gray-500 text-sm'>
                    @{post.user.username || 'anonymous'} • {moment(post.createdAt).fromNow()}
                </div>
            </div>
        </div>
        {/* Content */}
